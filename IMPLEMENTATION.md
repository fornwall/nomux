...
- The main loop polls stdin, the session socket and a non-blocking worker channel,
  ...
  bounded worker thread owns that write; its socketpair returns the copy result on a clean
  end, and an abruptly killed relay cannot orphan it — a process's exit takes its threads
  with it. A stalled pipe, terminal, socket or regular file thus backpressures session
  output without stopping stdin from reaching the session. The socketpair channel is the
  only additional fixed kernel buffer, and closing it both reports worker failure and
  flushes every accepted byte on a clean end.
