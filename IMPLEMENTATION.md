...
cost someone their session. A value mistyped *upwards* is clamped to 1 GiB rather
than fallen back from: it parses and is positive, so nothing rejects it, and
...
upload; nothing does that today** — where `SHA256SUMS` goes after this script writes
it, and what is missing before a client could read one, is
[PLAN.md § P3](PLAN.md#p3--release-process)'s. Release builds must pin a **dated**
